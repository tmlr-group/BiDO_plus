...
class NeuralLinear(object):
    ...
    '''
    def __init__(self, model, repr_dim, image_size, output_dim):
        #######
        a0 = 6.0
        b0 = 6.0
        sigma = 20
        ...
        self.a = torch.tensor([a0 for _ in range(self.output_dim)]).cuda()
        self.b = torch.tensor([b0 for _ in range(self.output_dim)]).cuda()
        # Update formula in BDQN
        self.sigma = sigma # W prior variance
        self.sigma_n = sigma_n # noise variacne
        ...
        self.beta_s = None
        self.train_x_ood = torch.empty(0, 3, image_size, image_size).cuda()
        self.train_x_id_base = torch.empty(0, 3, image_size, image_size).cuda()

    def update_representation(self):
        latent_z = torch.empty(0, self.repr_dim, device = "cuda")
        print('begin updating representation')
        data_loader = torch.utils.data.DataLoader(
                SimpleDataset(self.train_x.cpu(), self.train_y.cpu()),
                batch_size=256, shuffle = False, num_workers = 2)
        self.model.eval()
        with torch.no_grad():
            for images, _ in data_loader:
                partial_latent_z = self.model.embedding(images.cuda())
    ...
    def train_blr(self, train_loader_in, train_loader_out, criterion, optimizer, epoch, save_dir, log, energy_model = False):
        ...
        for i, (in_set, out_set) in enumerate(zip(train_loader_in, train_loader_out)):
            ...
            if epoch == 0:
                ...
                self.train_x_id_base = torch.cat((self.train_x_id_base, in_input), dim = 0) 

            if epoch < self.args.BUF_SIZE:
                # self.train_x_ood = torch.cat((self.train_x_ood, out_set[0]), dim = 0)
                self.train_x_ood = torch.cat((self.train_x_ood, out_input), dim = 0)

            else: 
                replace_idx = np.arange(start_idx, start_idx + out_len)
                self.train_x_ood[replace_idx] = out_input
                start_idx = start_idx + out_len

            self.model.train()

            cat_input = torch.cat((in_input, out_input), 0) 
            _, cat_output = self.model(cat_input)   
    ...
    def predict(self, x):
        latent_z = self.model.embedding(x)
