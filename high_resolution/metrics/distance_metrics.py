...
from datasets.custom_subset import SingleClassSubset, ClassSubset
...
class DistanceEvaluation():
    ...
    def prepare_dataset(self):
        ...
        if self.dataset_name == 'facescrub':
            ...
            train_set = CelebA1000(
                mode="train", transform=transform, split_seed=self.seed)
    ...
    def find_closest_training_sample(self, imgs, targets, batch_size=64):
        ...
        for img, target in zip(imgs, targets):
            ...
            target_subset = ClassSubset(self.train_set, target_classes=list(set(targets)))
            if len(img) == 3:
                img = img.unsqueeze(0)
            target_embeddings = []
            with torch.no_grad():
