...
class CelebA_OOD(Dataset):
    def __init__(self,
        ...
        if mode == "aux_ood":
            sorted_targets = list(ordered_dict.keys())[2000:4000]

        elif mode == "test_ood":
            sorted_targets = list(ordered_dict.keys())[4000:5000]
        ...
        if mode == "aux_ood":
            ...
            np.random.seed(split_seed)
            np.random.shuffle(indices)
            training_set_size = int(0.85 * len(indices))
            train_idx = indices[:training_set_size]
            test_idx = indices[training_set_size:]
...
class CustomCelebA(VisionDataset):
    ...
    def __init__(
        ...
        identity = pandas.read_csv(fn("identity_CelebA.txt"), sep='\s+', header=None, index_col=0)
        bbox = pandas.read_csv(fn("list_bbox_celeba.txt"), sep='\s+', header=1, index_col=0)
        landmarks_align = pandas.read_csv(fn("list_landmarks_align_celeba.txt"), sep='\s+', header=1)
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        file_path = os.path.join(self.root, "img_align_celeba224", self.filename[index])
        # file_path = os.path.join(self.root, "Img", self.filename[index])
