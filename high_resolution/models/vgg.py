...
class VGG16(nn.Module):
    def __init__(self, n_classes=1000, hsic_training=False):
        super(VGG16, self).__init__()

        self.hsic_training = hsic_training

        if self.hsic_training:
            blocks = make_layer(cfgs['D'], batch_norm=True)
        ...
        self.fc = nn.Linear(self.feat_dim, self.n_classes)
...
def _vgg(cfg: str, batch_norm: bool, weights: Optional[WeightsEnum], progress: bool, **kwargs: Any) -> VGG:
    ...
    if weights is not None:
        model_params = weights.get_state_dict(progress=progress)
        # del model_params['classifier.6.weight']
        # del model_params["classifier.6.bias"]
