...
class TrainingConfigParser:
    ...
    def create_model(self):
        ...
        bido_config = self._config['bido']
        OE_config = self._config['outlier_exp']

        print(model_config)
        print(bido_config)
        print(OE_config)
        print()
        model = Classifier(**model_config, bido_args=self._config['bido'], OE_args=self._config['outlier_exp'])
    ...
    def create_datasets(self):
        ...
        # ood_test_name = dataset_config['ood_type'].lower()
        aux_ood_name = self._config['outlier_exp']['aux_ood_dataset']

        # train_set, valid_set, test_set = None, None, None
        train_set, valid_set, test_set = None, None, None

        data_transformation_train = self.create_transformations(mode='training',
                                                                normalize=True)
        ...
        if name == 'facescrub':
            ...
            train_set = CelebA1000(mode="train")
            test_set = CelebA1000(mode="test",
                                  transform=data_transformation_test)
        ...
        aux_ood_set = [] 
        if aux_ood_name == "celeba_identities":
            aux_ood_set = CelebA_OOD(mode="aux_ood", transform=data_transformation_test)
        elif aux_ood_name == "facescrub":
            aux_ood_set = FaceScrub(group='all', train=True, cropped=True, transform=data_transformation_test)
        ...
        if validation_set_size > 0:
            valid_idx = indices[train_set_size:train_set_size + validation_set_size]
        ...
        print(
            f'Created {name} datasets with OOD dataset {aux_ood_name} {len(aux_ood_set):,} auxiliary ood samples.')
        
        return train_set, test_set, aux_ood_set
    ...
    @property
    def outlier_exp(self):
        return self._config['outlier_exp']

    @property
    def bido(self):
        return self._config['bido']
