...
from datasets.celeba import CelebA1000, CelebA_OOD
from datasets.facescrub import FaceScrub
from datasets.ffhq import FFHQ
from utils.datasets import get_normalization

import wandb
from utils.wandb import load_model, load_config
...
class AttackConfigParser:
    ...
    def create_datasets(self, mode="test_ood"):
        dataset_config = self._config['dataset']
        name = dataset_config['type'].lower()
        ood_name = dataset_config['ood_type'].lower()

        if name == 'celeba_identities':
            if mode == "test_ood":
                data_transformation_test = self.create_transformations(mode='test',
                                                                       normalize=True)
                test_id_set = CelebA1000(mode="test",
                                         transform=data_transformation_test)
                
                if ood_name == 'celeba_identities':
                    test_ood_set = CelebA_OOD(mode="test_ood",
                                              transform=data_transformation_test)
                elif ood_name == 'ffhq':
                    test_ood_set = FFHQ(transform=data_transformation_test)
                elif ood_name == 'facescrub':
                    test_ood_set = FaceScrub(group='all',
                                        train=True,
                                        cropped=True,
                                        transform=data_transformation_test)
                else:
                    raise RuntimeError('No OOD dataset stated in the config file.')
                
                return test_id_set, test_ood_set

        if name == 'ffhq':
            if mode == "KEDMI":
                data_transformation_gan = self.create_transformations(mode='KEDMI',
                                                                      normalize=False)
                gan_set = FFHQ(transform=data_transformation_gan)
        ...
        if name == 'facescrub':
            if mode == "test_ood":
                data_transformation_test = self.create_transformations(mode='test',
                                                                       normalize=True)
                test_id_set = FaceScrub(group='all',
                                        train=False,
                                        cropped=True,
                                        transform=data_transformation_test)
    ...
    def create_transformations(self, mode, normalize=True):
        ...
        if mode == 'training' and 'transformations' in self._config:
            ...
            transformation_list = []
            # resize images to the expected size
            transformation_list.append(T.Resize(256))
            transformation_list.append(T.ToTensor())
            transformation_list.append(T.CenterCrop((256, 256)))
    ...
    def get_training_config(self):
        if 'wandb_target_run' in self._config:
            run_path = self._config['wandb_target_run']
            api = wandb.Api(timeout=60)
            run = api.run(run_path)
            config_file = run.config["config_path"]
            training_config = load_config(run_path, config_file)

        elif 'config_path' in self._config:
            config_file = self._config["config_path"]
            with open(config_file, 'r') as file:
    ...
    def create_target_model(self):
        if 'wandb_target_run' in self._config:
            ...
            model = Classifier(num_classes=config['num_classes'],
                               ...
                               OE_args=self.target_model_config['outlier_exp'])
            ...
            try:
                tmp = model.load_state_dict(state_dict)
    ...
    def create_evaluation_model(self):
        if 'wandb_evaluation_run' in self._config:
            ...
            evaluation_model = Classifier(num_classes=config['num_classes'],
                                          architecture=config['architecture'])
            evaluation_model.load_state_dict(torch.load(config['weights'])["model_state_dict"])
        else:
            raise RuntimeError(
