...
def load_model(run_path,
    ...
    model = Classifier(num_classes, 
                       ...
                       bido_args=training_config["bido"],
                       OE_args=training_config["outlier_exp"]
                       )
    ...
    model.load_state_dict(torch.load(file_model.name, map_location='cpu')['model_state_dict'])
...
def log_nearest_neighbors(imgs, targets, eval_model, model_name, dataset, img_size, seed):
    # Find closest training samples to final results
    evaluater = DistanceEvaluation(
        eval_model, None, img_size, None, dataset, seed)
